use crate::{
    error::GlossaError::TextNotFound,
    resource::{loader::FluentLoader, LangResource},
};
...
impl<'a> LangResource<'a> {
    ...
    fn get_text<K: AsRef<str>>(
        ...
        match self.loader {
            ...
            Static(s) => s
                .lookup_single_language(lang, id, map) // Call `lookup_single_language` on the static loader.
                .ok_or(TextNotFound), // If the lookup fails, return an error.
            Arc(a) => a
                .lookup_single_language(lang, id, map) // Call `lookup_single_language` on the Arc loader.
                .ok_or(TextNotFound),
            ArcOwned(ref o) => o
                .lookup_single_language(lang, id, map)
                .ok_or(TextNotFound),
        }
    }
    ...
    fn find_from_fallback_chain<K: AsRef<str>>(
        ...
        self.set_chain_once(None)
            ...
            .ok_or(TextNotFound) // If no lookup succeeds, return an error.
    }
